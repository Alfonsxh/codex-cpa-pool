...
入口为“请求与账号 → 插件与版本检查”，支持 CPA / Codex Ticket 版本检查和按账号安装、升级插件。
...
- 先填写 `plugins.codex_ticket.accounts`（账号 ID，以逗号分隔）和适用模型，保存后按账号点击“安装 / 升级”。仅允许处理已启用且运行中的账号，停止账号不会被启动。镜像需提供原生插件接口；当前支持 linux/amd64 glibc。
